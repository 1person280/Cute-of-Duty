...
pub(crate) fn setup_minimap(mut commands: Commands) {
    ...
    commands.spawn((
        ...
        root.spawn((
            ...
            BorderColor::all(Color::srgba(0.6, 0.63, 0.67, 0.9)),
        )).with_children(|map| {
            ...
            for prop in &layout.props {
                ...
                map.spawn((
                    ...
                    BackgroundColor(minimap_material_color(prop.material)),
                ));
            }
            // 功能站点：补给台（琥珀）/ 干员切换台（青）
...
type MinimapDotStyleQuery<'w, 's> = Query<
    'w, 's,
    (&'static MinimapDot, &'static mut Node, &'static mut BackgroundColor),
    (
...
type MinimapPlayerArrowQuery<'w, 's> = Query<
    'w, 's,
    (&'static mut Node, &'static mut Transform),
    (
...
/// 各 Node 可变查询用互斥的 With/Without 标记隔离，避免 B0001 运行时冲突。
pub(crate) fn minimap_update_system(
    ...
    let Ok(cam) = world.cam_query.single() else { return };
    let Ok(player) = world.player_query.single() else { return };
    ...
    if let Ok(mut style) = player_dot.single_mut() {
        style.left = Val::Px(px - 2.0);
        style.top = Val::Px(py - 2.0);
    }
    if let Ok((mut style, mut transform)) = player_arrow.single_mut() {
