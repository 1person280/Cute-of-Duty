        ...
        overlay.spawn((
            ...
            panel.spawn((
                ...
                title_row.spawn((
                    ...
                    TextColor(Color::srgb(0.95, 0.78, 0.25)),
                    HudBackpackAmmo,
                ));
            });
            // Divider line
                ...
                for i in 0..8 {
                    grid.spawn((
                        ...
                        slot.spawn((
                            ...
                            TextColor(Color::srgb(0.85, 0.85, 0.85)),
                            Node { width: Val::Percent(90.0), height: Val::Auto, justify_content: JustifyContent::Center, align_items: AlignItems::Center, ..default() },
                            InventorySlotText(i),
                        ));
                    });
                }
            ...
            // Instructions
            panel.spawn((
                Text::new("Tab/Esc 关闭 · 悬停物资+R 使用 · 3/4 快捷 / 长按呼出轮盘（点击中心撤销）\n手雷使用后持握瞄准：左键投掷 / Esc 取消 · 右键越肩瞄准 · 拾取武器替换当前手持 · 弹药拾取直接入弹药池"),
                TextFont { font_size: FontSize::Px(12.0), ..default() },
                TextColor(Color::srgb(0.5, 0.5, 0.55)),
        ...
        parent.spawn((
            ...
            panel.spawn((
                ...
                header.spawn((
                    ...
                    BackgroundColor(Color::srgba(0.9, 0.85, 0.25, 0.9)),
                )).with_children(|badge| {
                    badge.spawn((
                        Text::new("F"),
                        TextFont { font_size: FontSize::Px(13.0), ..default() },
            ...
            panel.spawn((
                ...
                body.spawn((
                    ...
                    for i in 0..INTERACT_MENU_VISIBLE_ROWS {
                        list.spawn((
                            ...
                            BorderColor::all(Color::srgba(0.3, 0.3, 0.35, 0.4)),
                            Visibility::Hidden,
                            InteractRow(i),
                        )).with_children(|row| {
                            row.spawn((
                ...
                body.spawn((
                    ...
                    track.spawn((
                        ...
                        BackgroundColor(Color::srgba(0.6, 0.6, 0.65, 0.7)),
                        Visibility::Hidden,
...
pub(crate) fn inventory_toggle(
    ...
    if !tab && !esc { return; }
    let Ok(mut vis) = inventory_ui.single_mut() else { return };
    let Ok(mut cursor) = window_query.single_mut() else { return };
...
pub(crate) fn inventory_item_use_system(
    ...
    let Ok(vis) = inventory_ui.single() else { return };
    if *vis != Visibility::Visible { return; }
    if !keyboard.just_pressed(KeyCode::KeyR) { return; }

    let Ok((mut inventory, mut health, mut armor)) = player_query.single_mut() else { return };
...
>;
type WeaponRootQuery<'w, 's> = Query<
    'w, 's,
    (&'static BackpackWeaponText, &'static mut Text, &'static mut TextColor, &'static Children),
    (Without<InventorySlotText>, Without<HudBackpackAmmo>),
>;
type WeaponSpanQuery<'w, 's> = Query<
    'w, 's,
    (&'static mut TextSpan, &'static mut TextColor),
    Without<BackpackWeaponText>,
>;
...
pub(crate) fn inventory_ui_update(
    ...
    mut weapon_roots: WeaponRootQuery,
    mut weapon_spans: WeaponSpanQuery,
    ...
    for (bw, mut text, mut color, children) in weapon_roots.iter_mut() {
        if let Some(w) = inventory.weapons.get(bw.0) {
            text.0 = format!("[{}] {}", bw.0 + 1, w.name);
            color.0 = w.element.color();
            if let Some(ammo_child) = children.first() {
                if let Ok((mut span, mut scolor)) = weapon_spans.get_mut(*ammo_child) {
                    span.0 = format!("\n{}/{} 弹", w.ammo, w.max_ammo);
                    scolor.0 = Color::srgb(0.8, 0.8, 0.82);
                }
            }
        } else {
            text.0 = "空".to_string();
            ...
            if let Some(ammo_child) = children.first() {
                if let Ok((mut span, _)) = weapon_spans.get_mut(*ammo_child) {
                    span.0 = String::new();
                }
            }
        }
