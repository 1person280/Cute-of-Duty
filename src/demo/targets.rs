...
pub(crate) fn target_dummy_logic(
    ...
    for (_entity, mut dummy, mut transform) in query.iter_mut() {
        ...
        dummy.dots.retain(|dot| !dot.remaining.is_finished());
        ...
        if let Some(ref mut down) = dummy.down_timer {
            ...
            // 同时清空持续效果（点燃熄灭、冰块碎裂）
            kill_events.write(KillEvent { name: dummy.label.to_string() });
...
pub(crate) fn moving_target_logic(
    ...
    for (mut transform, mut target, dummy) in query.iter_mut() {
        ...
        transform.translation.x += target.direction * target.speed * time.delta_secs();
    }
}
...
pub(crate) fn crosshair_hit_feedback(
    ...
    let Ok(movement) = player_query.single_mut() else { return };
    let t = movement.shoot_cooldown.elapsed_secs() / movement.shoot_cooldown.duration().as_secs_f32();
    // 命中反馈（红） > 瞄准态（琥珀） > 常态（白）
    let aiming = cam_query.single().map(|c| c.aim_lerp > 0.35).unwrap_or(false);
    let color = if t < 0.3 {
