...
use crate::combat::{CombatIntent, CombatSystem};
...
impl GameLoop {
    ...
    /// 执行单个Tick
    ///
    /// `pub` 供服务端权威主循环逐帧驱动：每个固定 Tick 调一次本方法推进确定性模拟，
    /// 随后立即由网络层基于 `world()`/`current_tick()` 构建并广播权威快照，
    /// 让「模拟在服务端、客户端只收结果」的服务器权威架构落地。
    ...
    /// 推进世界内所有战斗对象：手雷飞行/引爆、毒区周期结算。
    fn run_combat(&mut self, delta_time: f32) {
        let env = self.environment.clone();
        let resolver = self.damage_resolver.clone();
        let combat = &mut self.combat;
        let world = &mut self.world;
        combat.tick_world(world, &resolver, delta_time, &env);
    }

    /// 结算单个玩家的本帧战斗意图（开火/换弹/技能）。
    ///
    /// 服务端主循环在每 Tick 消费输入后调用；命中/击杀压入事件队，
    /// 由 [`Self::drain_combat_events`] 出队转播报。
    pub fn apply_combat_input(&mut self, eid: EntityId, intent: CombatIntent) {
        let env = self.environment.clone();
        let resolver = self.damage_resolver.clone();
        let dt = 1.0 / self.config.tick_rate_hz.max(1) as f32;
        let combat = &mut self.combat;
        let world = &mut self.world;
        combat.apply_input(world, &resolver, eid, dt, &env, intent);
    }

    /// 出队所有未播报的战斗事件（击杀/命中）。
    pub fn drain_combat_events(&mut self) -> Vec<crate::combat::CombatEvent> {
        self.combat.drain_events()
    }
