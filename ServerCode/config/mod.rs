...
///
/// 自 0.6.0 迁入 Cargo Workspace 后，配置表与加载器位于服务端 crate 内，
/// 统一以 **workspace 根为锚点**（`ServerCode/config/element_reactions.yaml`），
/// 使 `project_root()` 无论从 exe 目录还是任意工作目录启动都能逐级向上定位到根。
/// 这也保证客户端(仅引 protocol)不会参与配置加载，配置归属服务端权威。
pub const ELEMENT_CONFIG_REL: &str = "ServerCode/config/element_reactions.yaml";
